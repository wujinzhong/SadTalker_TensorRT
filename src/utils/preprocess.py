...
class CropAndExtract():
    def __init__(self, sadtalker_path, device):
        ...
        self.net_recon_trt_engine = None
    ...
    def generate(self, input_path, save_dir, crop_or_resize='crop', source_image_flag=False, pic_size=256,
        ...
        with NVTXUtil("load images", "blue", mm), SynchronizeUtil(torchutil.torch_stream):
            #load input
        ...
        if 'crop' in crop_or_resize.lower(): # default crop
            with NVTXUtil("crop", "blue", mm), SynchronizeUtil(torchutil.torch_stream):
        ...
        with NVTXUtil("imwrite", "blue", mm), SynchronizeUtil(torchutil.torch_stream):
            # save crop info
        ...
        if True:
            with NVTXUtil("extract_keypoint", "blue", mm), SynchronizeUtil(torchutil.torch_stream):
                print(f"self.propress.predictor.extract_keypoint......")
        ...
        if True:
            ...
            for idx in tqdm(range(len(frames_pil)), desc='3DMM Extraction In Video:'):
                ...
                with torch.no_grad():
                    if self.net_recon_trt_engine:
                        with NVTXUtil("net_recon_trt", "blue", mm), SynchronizeUtil(torchutil.torch_stream):
                            trt_output = self.net_recon_trt_engine.inference(inputs=[im_t.contiguous(),],
                                                                            outputs = self.face_parse_trt_engine.output_tensors)
                            full_coeff = trt_output[0]
                    else:
                        with NVTXUtil("net_recon_torch", "blue", mm), SynchronizeUtil(torchutil.torch_stream):
                            full_coeff = self.net_recon(im_t)
                            print(f"im_t: {im_t.shape, im_t.device, im_t.dtype}")
                            print(f"full_coeff: {full_coeff.shape, full_coeff.device, full_coeff.dtype}")
