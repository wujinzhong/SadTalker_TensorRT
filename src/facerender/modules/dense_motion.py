...
class DenseMotionNetwork(nn.Module):
    ...
    def create_sparse_motions(self, feature, kp_driving_value, kp_source_value):
        bs, _, d, h, w = feature.shape
        identity_grid = make_coordinate_grid((d, h, w), type=kp_source_value.type())
        identity_grid = identity_grid.view(1, 1, d, h, w, 3)
        coordinate_grid = identity_grid - kp_driving_value.view(bs, self.num_kp, 1, 1, 1, 3)
        ...
        driving_to_source = coordinate_grid + kp_source_value.view(bs, self.num_kp, 1, 1, 1, 3)    # (bs, num_kp, d, h, w, 3)
    ...
    def create_heatmap_representations(self, feature, kp_driving_value, kp_source_value):
        spatial_size = feature.shape[3:]
        gaussian_driving = kp2gaussian(kp_driving_value, spatial_size=spatial_size, kp_variance=0.01)
        gaussian_source = kp2gaussian(kp_source_value, spatial_size=spatial_size, kp_variance=0.01)
    ...
    def forward(self, feature, kp_driving_value, kp_source_value):
        bs, _, d, h, w = feature.shape
        ...
        out_dict = dict()
        sparse_motion = self.create_sparse_motions(feature, kp_driving_value, kp_source_value)
        deformed_feature = self.create_deformed_feature(feature, sparse_motion)

        heatmap = self.create_heatmap_representations(deformed_feature, kp_driving_value, kp_source_value)
