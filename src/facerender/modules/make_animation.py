...
def make_animation(source_image, source_semantics, target_semantics,
    ...
    with torch.no_grad():
        ...
        for frame_idx in tqdm(range(target_semantics.shape[1]), 'Face Renderer:'):
            ...
            rng = nvtx.start_range( message="keypoint_transformation", color='red' )
            ...
            if generator_trt_engine:
                ...
                trt_output = generator_trt_engine.inference(inputs=[source_image.contiguous(), 
                                                                    kp_source['value'].contiguous(), 
                                                                    kp_norm['value'].contiguous()],
                                                            outputs = generator_trt_engine.output_tensors)
                out = trt_output
                out_mask = out[0]
            ...
            '''
            #print(f"generator inputs/output------------------------------------")
            #print(f"source_image: {source_image.shape, source_image.device, source_image.dtype}")
            #print(f"kp_source['value']: {kp_source['value'].shape, kp_source['value'].device, kp_source['value'].dtype}  ")
            #print(f"kp_norm['value']: {kp_norm['value'].shape, kp_norm['value'].device, kp_norm['value'].dtype}  ")
            #print(f"out_mask: {out_mask.shape, out_mask.device, out_mask.dtype}  ")
            #print(f"out_occlusion_map: {out_occlusion_map.shape, out_occlusion_map.device, out_occlusion_map.dtype}  ")
            #print(f"out_prediction: {out_prediction.shape, out_prediction.device, out_prediction.dtype}  ")
