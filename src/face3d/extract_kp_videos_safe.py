            ...
            while True:
                try:
                    with torch.no_grad():
                        ...
                        print(f"det_net.detect_faces......")
                        if isinstance(images, Image.Image):
                            images = torch.from_numpy(images).to(device='cuda')
                            images[:,0],images[:,2] = images[:,2],images[:,0] #image = cv2.cvtColor(np.asarray(image), cv2.COLOR_RGB2BGR)

                        bboxes = self.det_net.detect_faces(images, 0.97)
                        
                        bboxes = bboxes[0]
                        img = img[int(bboxes[1]):int(bboxes[3]), int(bboxes[0]):int(bboxes[2]), :]
                        self.predictor.detector.to(device='cuda')
                        keypoints = landmark_98_to_68(self.predictor.detector.to(device='cuda').get_landmarks(img)) # [0]
