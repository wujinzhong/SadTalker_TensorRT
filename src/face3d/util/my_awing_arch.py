...
class FAN(nn.Module):
    ...
    def get_landmarks(self, img):
        ...
        if self.trt_engine: 
            rng = nvtx.start_range(message="my_awing_arch.FAN.forward/trt")
            trt_output = self.trt_engine.inference(inputs=[inp.contiguous(),],
                                                            outputs = self.trt_engine.output_tensors)
            output = trt_output[0]
        else:
            rng = nvtx.start_range(message="my_awing_arch.FAN.forward/torch")
        ...
        '''
        #print(f"inp: {inp.shape, inp.device, inp.dtype}")
