            ...
            with warnings.catch_warnings():
                ...
                if True:
                    torch.onnx.export(
                        ...
                        onnx_model_path, #f,
                        export_params=True,
                        verbose= True if MY_VERBOSE else False,
                        opset_version=18,
...
if trt_engine is None: 
    ...
    trt_engine = TRT_Engine(trt_engine_path, gpu_id=0, torch_stream=torch_stream,
                                        onnx_inputs_name = inputs_name,
                                        onnx_outputs_name = outputs_name,)
    assert trt_engine
    trt_engine = trt_engine if USE_TRT else None
    ......
    if trt_engine:
        trt_output = trt_engine.inference(inputs=[midas_input.to(torch.float32).contiguous()],
                                                  outputs = trt_engine.output_tensors)
        output = trt_output[0].to(torch.float16)
    else:
...
class TRT_Engine():
    ...
    def _init_execution_context( self, trt_engine_path ):
        if True: # deserialize TRT engines
            ...
            if MY_VERBOSE: print(f"{trt_engine_path}: {self.engine}")
        ...
        if isinstance(inputs, list) and len(inputs)>0 and torch.is_tensor(inputs[0]):
            ...
            if torch.is_tensor(inputs[0]) and torch.is_tensor(outputs[0]) and inputs[0].is_cuda and outputs[0].is_cuda:
                self.inference_from_torch_tensor_external_inputs_outputs( self.trt_exec_context, 
                                                                inputs, outputs, 
                                                                self.torch_stream.cuda_stream if torch_stream==None else torch_stream.cuda_stream)
                ...
                if self.onnx_outputs_name is not None:
                    ...
                    for i in range(len(self.onnx_outputs_name)):
                        if self.onnx_outputs_name[i] in self.outputs_name:
                            ...
                            print(f"{self.onnx_outputs_name[i]} is not in {self.outputs_name}")
                            assert False
...
class SynchronizeUtil:
    def __init__(self, torch_stream=None, is_in_sync=True, is_out_sync=True):
        self.torch_stream = torch_stream
        self.is_in_sync = is_in_sync
        self.is_out_sync = is_out_sync
        
    def __enter__(self):
        if self.is_in_sync: synchronize( self.torch_stream )
        return None
    def __exit__(self, exceptionType, exceptionVal, trace):
        if self.is_out_sync: synchronize( self.torch_stream )
