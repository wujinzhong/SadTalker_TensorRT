...
def landmark_98_to_68(landmark_98):
    ...
    Returns:
        landmark_68(numpy array): Polar coordinates of 98 landmarks, (68, 2)
    """
    landmark_98 = torch.from_numpy(landmark_98).to(device='cuda')
    landmark_68 = torch.zeros((68, 2), dtype=torch.float32, device='cuda')
    # cheek
    for i in range(0, 33):
        if i % 2 == 0:
            landmark_68[(i // 2), :] = landmark_98[i, :]
    # nose
    for i in range(51, 60):
        landmark_68[i - 24, :] = landmark_98[i, :]
    # mouth
    ...
    landmark_68[26, :] = landmark_98[46, :]
    # left eye
    LUT_landmark_68_left_eye = torch.tensor([36, 37, 38, 39, 40, 41], dtype=int, device='cuda')
    LUT_landmark_98_left_eye = torch.tensor([60, 61, 63, 64, 65, 67], dtype=int, device='cuda')
    for idx, landmark_98_index in enumerate(LUT_landmark_98_left_eye):
        landmark_68[LUT_landmark_68_left_eye[idx], :] = landmark_98[landmark_98_index, :]
    # right eye
    LUT_landmark_68_right_eye = torch.tensor([42, 43, 44, 45, 46, 47], dtype=int, device='cuda')
    LUT_landmark_98_right_eye = torch.tensor([68, 69, 71, 72, 73, 75], dtype=int, device='cuda')
    for idx, landmark_98_index in enumerate(LUT_landmark_98_right_eye):
        landmark_68[LUT_landmark_68_right_eye[idx], :] = landmark_98[landmark_98_index, :]

    return landmark_68.cpu().numpy()
